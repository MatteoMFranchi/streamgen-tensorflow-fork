...
public class ImageRecognizerTF implements Serializable {
	...
	ImageRecognizerTF(){	
	 ...
	 public Tensor<Float> constructAndExecuteGraphToNormalizeImage(byte[] imageBytes) {
		  ...
		  float[] executeInceptionGraph(byte[] graphDef, Tensor<Float> image) {
		    try (Graph g = new Graph()) {
		  ...
		  int[] bestThreeProbabilities(float[] probabilities) {
			  int[] results = new int[3];
			  float[] temp = probabilities;
			  int best = 0;
			  for(int j = 0; j < 3; j++) {
